    ...
    function EdytujProdukt() {
        ...
        if($id > 0) {
            ...
            if($product = $result->fetch_assoc()) {
                ...
                echo '<h3 class="edit-title">Edytuj produkt</h3>';
                echo '<form method="POST" enctype="multipart/form-data" class="product-form">';
                echo '<input type="hidden" name="id" value="' . $product['id'] . '">';
                echo '<div class="form-group">';
                echo '<label for="tytul">Tytuł:</label>';
                echo '<input type="text" id="tytul" name="tytul" value="' . htmlspecialchars($product['tytul']) . '" maxlength="255" required>';
                echo '</div>';
                echo '<div class="form-group">';
                echo '<label for="opis">Opis:</label>';
                echo '<textarea id="opis" name="opis" required>' . htmlspecialchars($product['opis']) . '</textarea>';
                echo '</div>';
                echo '<div class="form-group">';
                echo '<label for="cena_netto">Cena netto:</label>';
                echo '<input type="number" id="cena_netto" step="0.01" name="cena_netto" value="' . $product['cena_netto'] . '" required>';
                echo '</div>';
                echo '<div class="form-group">';
                echo '<label for="podatek_vat">VAT (%):</label>';
                echo '<input type="number" id="podatek_vat" name="podatek_vat" value="' . $product['podatek_vat'] . '" required>';
                echo '</div>';
                echo '<div class="form-group">';
                echo '<label for="ilosc_dostepnych">Ilość:</label>';
                echo '<input type="number" id="ilosc_dostepnych" name="ilosc_dostepnych" value="' . $product['ilosc_dostepnych'] . '" required>';
                echo '</div>';
                echo '<div class="form-group">';
                echo '<label for="status_dostepnosci">Status dostępności:</label>';
                echo '<select id="status_dostepnosci" name="status_dostepnosci">';
                ...
                echo '</div>';
                echo '<div class="form-group">';
                echo '<label for="gabaryt_produktu">Gabaryt:</label>';
                echo '<select id="gabaryt_produktu" name="gabaryt_produktu">';
                ...
                echo '<input type="datetime-local" id="data_wygasniecia" name="data_wygasniecia" value="' . 
                     date('Y-m-d\TH:i', strtotime($product['data_wygasniecia'])) . '" required>';
                echo '</div>';
                echo '<div class="form-group">';
                echo '<label>Aktualne zdjęcie:</label>';
                if($product['zdjecie']) {
                ...
                echo '</div>';
                echo '<div class="form-group">';
                echo '<label for="zdjecie">Nowe zdjęcie:</label>';
                echo '<input type="file" id="zdjecie" name="zdjecie" accept="image/*">';
                echo '</div>';
                echo '<div class="form-actions">';
