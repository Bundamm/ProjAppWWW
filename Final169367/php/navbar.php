...
class Navbar {
     ...
     * Strony są podzielone na główne i podstrony z grami (w dropdownie).
    ...
    function generateNavbar() {
        ...
        // Inicjalizuje tablice dla stron głównych i podstron z grami
        $mainPages = array();
        $gamePages = array();
        $specialPages = array(1, 8, 9);
        ...
        // Dodaje dropdown z grami, jeśli istnieją jakieś gry
        if(!empty($gamePages)) {
            $nav .= '<li class="dropdown' . (in_array($currentPage, array_column($gamePages, 'id')) ? ' active' : '') . '">';
            $nav .= '<a href="#" class="dropbtn">Gry</a>';
            $nav .= '<div class="dropdown-content">';
            
            // Sortuje gry alfabetycznie
            usort($gamePages, function($a, $b) {
                return strcmp($a['page_title'], $b['page_title']);
            });
            
            // Dodaje każdą grę do dropdown menu
